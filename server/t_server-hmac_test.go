...
func TestServHMACTCPServerer(t *testing.T) {
	...
	msg = <-as.Msgr
	if msg.Code != p.Stats["badmac"].Code {
		t.Errorf("msg.Code should have been %d but got: %d", p.Stats["badmac"].Code, msg.Code)
	}
	// shut down petrel
	as.Quit()
}
...
func echoHMACTCPclient(sn string, t *testing.T) {
	...
	if err.(*p.Status).Code != p.Stats["badmac"].Code {
		t.Errorf("err.Code should be %d but is %v", p.Stats["badmac"].Code, err.(*p.Status).Code)
	}
	if err.(*p.Status).Txt != p.Stats["badmac"].Txt {
		t.Errorf("err.Txt should be %s but is %v", p.Stats["badmac"].Txt, err.(*p.Status).Txt)
	}
}

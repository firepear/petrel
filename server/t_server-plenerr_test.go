...
func reqtests(as *Server, t *testing.T) {
	...
	msg = <-as.Msgr
	if msg.Txt != p.Stats["plenex"].Txt {
		t.Errorf("expected %s but got: %s", p.Stats["plenex"].Txt, msg.Txt)
	}
	if msg.Code != 402 {
...
func reqclient(sn string, t *testing.T) {
	...
	if err.(*p.Status).Code != p.Stats["plenex"].Code {
		t.Errorf("err.Code should be %d but is %v", p.Stats["plenex"].Code, err.(*p.Status).Code)
	}
	if err.(*p.Status).Txt != p.Stats["plenex"].Txt {
		t.Errorf("err.Txt should be %s but is %v", p.Stats["plenex"].Txt, err.(*p.Status).Txt)
	}
}

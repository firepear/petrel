...
func TestServENOLISTENER(t *testing.T) {
	...
	c := &ServerConfig{Sockname: "/tmp/test06-1.sock", Msglvl: "debug"}
	as, err := DeadUnix(c)
	if err != nil {
...
func TestServENOLISTENER2(t *testing.T) {
	// implement an echo server
	c := &ServerConfig{Sockname: "/tmp/test06-2.sock", Msglvl: "debug"}
	as, err := DeadUnix(c)
	if err != nil {
		t.Errorf("Couldn't create socket: %v", err)
	}
	...
	as.Quit()
	c = &ServerConfig{Sockname: "/tmp/test06-3.sock", Msglvl: "debug"}

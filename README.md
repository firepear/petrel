...
to deliver code that is well-tested and does what I mean it to do. In
addition to these badges, which let you know that (1) the `main`
branch builds cleanly, and (2) the project structure is sane:
...
- The test coverage report, which is generated by my `pre-commit` hook
- Speaking of `pre-commit`, it also runs
  - `gofmt`
  - `go vet`
  - `golangci-lint`
  - `staticcheck`
  - and the project test suite

I can't make a single commit if any of those fail. That doesn't mean
no bugs, but it does mean the code is free of a lot of bad smells, and
any bugs that I've seen before.
